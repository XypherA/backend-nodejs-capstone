...
router.post('/', upload.single('file'), async (req, res, next) => {
  try {
    ...
    // If there’s no existing item, start with id = 1
    ...
    const insertedItem = await collection.insertOne(secondChanceItem);

    // Step 3: task 7 - Image is automatically handled by multer (upload.single('file'))
    res.status(201).json(insertedItem.ops[0]);
  } catch (e) {
    next(e);
  }
});
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    // Step 6: task 3 - Find the item by ID and check if it exists
    const id = req.params.id;
    const secondChanceItem = await collection.findOne({ id });
    if (!secondChanceItem) {
      logger.error('secondChanceItem not found');
    ...
    // Step 6: task 4 - Delete the item and send confirmation
